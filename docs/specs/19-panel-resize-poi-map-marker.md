...
- [ ] AC1: 데스크톱에서 핸들 드래그 시 `left-panel` 너비가 실시간으로 변경됨
- [ ] AC2: 드래그 후 지도가 새 크기에 맞게 재렌더링됨 (흰 공백 없음)
- [ ] AC3: `left-panel` ≥ 280px, `map-panel` ≥ 320px 제약 적용됨
- [ ] AC4: 도보 시설 행 클릭 시 지도에 마커가 표시됨
- [ ] AC5: 마커 클릭 시 시설명 + 거리 InfoWindow 표시
- [ ] AC6: 다른 시설 클릭 시 이전 마커 제거 후 새 마커 표시
- [ ] AC7: 상세 패널 닫힐 때 POI 마커 모두 제거됨
- [ ] AC8: 장소 검색 결과 없을 때 앱이 중단되지 않음
