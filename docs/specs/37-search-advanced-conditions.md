...
> 상태: Draft | 작성일: 2026-06-14 | 브랜치: hjkang83
...
- [ ] AC1: 통근시간 아래 "상세조건" 접이식 섹션이 있고 기본 접힘 (기존 주소·가격·통근 화면 그대로)
- [ ] AC2: 평형 체크박스(기본 전체), 준공연도·세대수 라디오 선택 가능
- [ ] AC3: 분석하기 누르면 선택한 평형/준공연도/세대수가 result URL → `/api/search`에 반영되어 그 조건으로 분석됨
- [ ] AC4: 상세조건 미사용 시 현재와 동일하게 동작 (평형 전체, 준공/세대수 무제한)
- [ ] AC5: "조건 다시 입력" 시 상세조건 선택값 복원
- [ ] AC6: 평형 0개 선택 시 전체로 폴백
- [ ] 순수 프론트(search.html) — 백엔드/result 변경 없음, 모바일/데스크톱 정상
