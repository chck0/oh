...
- 조치 필요 항목: 환경변수 미설정으로 두 스크립트 모두 실행 실패
  - `KAKAO_REST_API_KEY` 누락으로 config.py 로드 자체가 실패
  - 리포 환경(CI/클라우드)에 `.env` 또는 Secrets 설정 필요: `KAKAO_REST_API_KEY`, `ODSAY_KEY_*`, `DATABASE_URL`
